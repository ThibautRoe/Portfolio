...
export default function ProfilePictureOnNameHover({ name }) {
    ...
    return (
      <div
        ...
        onMouseEnter={() => {
          if (!touchOnlyDevice) setShowProfilePicture(true)
        }}
        onMouseLeave={() => {
          if (!touchOnlyDevice) setShowProfilePicture(false)
        }}
      >
        <AnimatePresence>
          {showProfilePicture && (
            <div className="absolute bottom-s-fl-xl left-1/2 -translate-x-1/2 z-30 w-[9.5em] aspect-square">
              ...
              >
                <Image
                  src={ProfilePicture}
                  alt="Photo de profil de Thibaut Roegiers"
                />
              </motion.div>
            </div>
          )}
        ...
        <InView>
          {({ inView, ref, entry }) => (
            <span
              ...
              className={`font-bold ${
                inView
                  ? "not-touch-only-device:animate-[neonAnimation_12s_ease-in-out_infinite]"
                  : ""
              }`}
